...
LONG PATHS: Perspective is that nodes only exist at intersections, with paths inbetween
(vice SHORT PATHS: In a grid, each point is a node with a short path to neighbors - may be slower at large grid sizes)
'''
...
class Grid():
    ...
    def find_nodes(self):
        ...
        (Here, Node Criteria implies CORNERS, CROSSES, DEAD ENDS only)'''
        ...
        for p in points:
            ...
            if p.is_scaff:
                p.is_path = (nbr_vals[0] == nbr_vals[2] = '#') ^ (nbr_vals[1] == nbr_vals[3] == '#') 
    ...
    def find_paths(self):
        ...
        for i, point_row in enumerate(grid):
            ...
            for n in range(1, len(node_row)):
                ...
                if all([p.is_scaff for p in span]):
                    link = next(links)
                    paths[link] = ('H', (nodeR.row - nodeL.row), nodeL.node_id, nodeR.node_id)
        ...
        for i,p in enumerate(grid[0]):
            ...
            for n in range(1, len(node_col)):
                ...
                span = point_col[nodeL.row : 1+nodeR.row]
                if all([p.is_scaff for p in span]):
                    link = next(links)
                    link_length = (nodeR.col - nodeL.col)
                    paths[link] = ('V', link_length , nodeL.node_id, nodeR.node_id)
                    nodeL.paths.append((link, link_length, nodeR.node_id))
                    nodeR.paths.append((link, link_length, nodeL.node_id))
